...
export default function ShopsPage() {
  ...
  const load = () => shopsApi.list().then(setShops).catch(() => setShops([]));
  useEffect(() => { load(); }, []);

  const markers: MapMarker[] = shops
    .filter((s) => s.latitude != null && s.longitude != null)
    .map((s) => ({ id: s.id, lat: Number(s.latitude), lng: Number(s.longitude), label: s.name, color: "#10b981" }));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Agents pin shop locations on the map.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-[420px_1fr] gap-4 h-[calc(100vh-12rem)]">
        <Card className="overflow-hidden flex flex-col">
          <CardHeader className="pb-2"><CardTitle className="text-base">List ({shops.length})</CardTitle></CardHeader>
          <CardContent className="p-0 overflow-auto">
            <ul className="divide-y">
              {shops.map((s) => (
                <li key={s.id} className="p-4 flex items-start gap-3 hover:bg-accent/40">
                  <MapPin className="h-4 w-4 mt-1 text-primary" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <div className="font-medium truncate">{s.name}</div>
                      {s.code && <Badge variant="secondary" className="text-xs">{s.code}</Badge>}
                    </div>
                    <div className="text-xs text-muted-foreground truncate">{s.address ?? "No address"}</div>
                    {s.latitude == null && (
                      <div className="text-xs text-amber-600 mt-1">No coordinates</div>
                    )}
                  </div>
              ...
              {shops.length === 0 && (
                <li className="p-6 text-center text-sm text-muted-foreground">No shops yet</li>
              )}
            </ul>
          </CardContent>
        </Card>
...
}: { open: boolean; onOpenChange: (v: boolean) => void; initial: Shop | null; onSaved: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>{initial ? "Edit shop" : "New shop"}</DialogTitle></DialogHeader>
        <div className="grid gap-3">
          ...
          <div className="grid grid-cols-[1fr_1fr_auto] gap-2 items-end">
            ...
            <Button type="button" variant="outline" onClick={pickHere} className="gap-1">
              <Locate className="h-4 w-4" /> Here
            </Button>
          </div>
          <div>
