...
import { Search, Save, RefreshCw, DollarSign, Store, Package, X } from "lucide-react";
import { cn } from "@/lib/utils";

type Shop    = { id: string; name: string; code?: string; address?: string };
type Product = { id: string; sku: string; name: string; defaultPrice?: number; unit?: string };
type PriceLine = {
  productId: string;
  productName: string;
  productSku: string;
...
function loadPrices(): Record<string, Record<string, { price: number; note: string }>> {
  try {
...
function savePrices(data: Record<string, Record<string, { price: number; note: string }>>) {
...
export default function ShopPricingPage() {
  ...
  const [prices,   setPrices]   = useState<Record<string, Record<string, { price: number; note: string }>>>(loadPrices);
  ...
  useEffect(() => {
    if (!shopId || products.length === 0) { setLines([]); return; }
  ...
  function updateLine(productId: string, field: "customPrice" | "note", value: string) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.success(`Price list saved for ${shops.find((s) => s.id === shopId)?.name}`);
    } finally {
      setSaving(false);
    }
  }
  ...
  function clearShopPrices() {
    ...
    toast.success("Price overrides cleared");
  }
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-xl font-semibold tracking-tight flex items-center gap-2">
            <DollarSign className="h-5 w-5" /> Shop Price Lists
          </h2>
        ...
        <div className="flex items-center gap-2">
          {shopId && changedCount > 0 && (
            <Badge variant="secondary" className="text-xs">{changedCount} overrides</Badge>
          )}
          {shopId && (
            <Button variant="outline" size="sm" onClick={clearShopPrices} className="gap-1.5 text-destructive hover:text-destructive">
              <X className="h-3.5 w-3.5" /> Clear
            </Button>
          )}
          <Button size="sm" onClick={handleSave} disabled={saving || !shopId} className="gap-1.5">
          ...
          <div className="grid sm:grid-cols-[1fr_2fr] gap-4 items-center">
            <div className="space-y-1.5">
              <Label className="text-xs flex items-center gap-1">
                <Store className="h-3.5 w-3.5" /> Select shop
              </Label>
              <Select value={shopId || "__none__"} onValueChange={(v) => setShopId(v === "__none__" ? "" : v)}>
                <SelectTrigger className="h-9">
                  <SelectValue placeholder="Choose a shop to set prices…" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none__">— Choose shop —</SelectItem>
                  {shops.map((s) => (
                  ...
                  {shops.length === 0 && (
                    <div className="px-3 py-2 text-xs text-muted-foreground">No shops found</div>
                  )}
                </SelectContent>
              </Select>
            </div>
            {selectedShop && (
              <div className="text-sm text-muted-foreground">
                ...
                {selectedShop.address && <p className="text-xs">{selectedShop.address}</p>}
                <p className="text-xs mt-0.5">
                  {(prices[shopId] ? Object.keys(prices[shopId]).length : 0)} price overrides saved
                </p>
              </div>
            )}
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/40">
                      ...
                      <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">Product</th>
                      <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">Unit</th>
                      <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">Default price</th>
                      <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground w-36">Custom price</th>
                      <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground w-40">Note</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {filtered.map((line) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-3 py-2 font-mono text-xs text-muted-foreground">{line.productSku}</td>
                          <td className="px-3 py-2">
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-sm truncate max-w-[180px]">{line.productName}</span>
                              {hasOverride && (
                                <Badge variant="outline" className="text-[9px] text-primary border-primary/40 shrink-0">custom</Badge>
                              )}
                            </div>
                          </td>
                          <td className="px-3 py-2 text-xs text-muted-foreground">{line.unit}</td>
                          <td className="px-3 py-2 text-sm font-mono">
                            {line.defaultPrice > 0 ? line.defaultPrice.toFixed(2) : "—"}
                          </td>
                          <td className="px-3 py-2">
                            <Input
                              type="number"
                              ...
                              onChange={(e) => updateLine(line.productId, "customPrice", e.target.value)}
                              placeholder={line.defaultPrice > 0 ? String(line.defaultPrice) : "0.00"}
                              className={cn(
