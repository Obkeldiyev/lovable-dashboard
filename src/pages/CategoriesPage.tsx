...
function CreateCategoryDialog({
  ...
  useEffect(() => {
    ...
    api.get("/api/categories")
      .then(({ data }) => setExisting(Array.isArray(data?.data) ? data.data : []))
      .catch(() => setExisting([]));
  }, [open]);
              ...
              <SelectContent>
                ...
                {existing.length === 0 && (
                  <div className="px-3 py-2 text-xs text-muted-foreground">
                    No categories yet — this will be the first
                  </div>
                )}
              </SelectContent>
            </Select>
            <p className="text-xs text-muted-foreground">
              Leave empty to create a top-level category (e.g. "Electronics").
              Pick a parent to create a sub-category (e.g. "Phones" under "Electronics").
            </p>
          </div>

          {/* Path — optional, auto-suggest based on parent */}
          <div className="space-y-1.5">
            <Label>Path <span className="text-muted-foreground text-xs font-normal">(optional)</span></Label>
            <Input
              value={form.path}
              onChange={(e) => set("path", e.target.value)}
              placeholder={
                form.parentId
                  ? `e.g. ${catLabel(existing.find((c) => c.id === form.parentId) ?? { name: "Parent" })} / ${form.name || "Sub"}`
                  : "e.g. Electronics"
              }
              className="h-9"
            />
    ...
    <div>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Product category tree</p>
        </div>
